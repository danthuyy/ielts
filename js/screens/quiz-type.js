...
export async function render(container, params = {}) {
  ...
  const renderQuestion = () => {
    ...
    container.innerHTML = `
      <div class="screen-quiz-type" style="display: flex; flex-direction: column; height: 100%; background: var(--bg);">
        ...
        <div style="flex: 1; padding: 24px; display: flex; flex-direction: column; justify-content: center; align-items: center;">
          <div style="text-align: center; margin-bottom: 32px;">
            ...
            ${currentWord.collocation ? `<div style="font-size: 14px; color: var(--warning); margin-top: 8px;">${currentWord.collocation}</div>` : ''}
          </div>

          <div style="width: 100%; max-width: 400px; margin-bottom: 16px;">
            <input type="text" id="type-input" autocomplete="off" placeholder="${hint} (${currentWord.word.length} chữ cái)" style="width: 100%; padding: 16px; font-size: 20px; text-align: center; border-radius: 12px; border: 2px solid var(--surface); background: var(--card); color: var(--text-primary); outline: none; box-sizing: border-box;">
          </div>
          
          <div id="feedback-area" style="min-height: 60px; width: 100%; max-width: 400px; text-align: center; margin-bottom: 24px; display: flex; flex-direction: column; justify-content: center;"></div>
    ...
    const checkAnswer = async () => {
      ...
      await Store.recordActivity(1, isCorrect ? 1 : 0, 'quiz-type');

      if (isCorrect) {
        correctCount++;
        input.style.borderColor = 'var(--success)';
        input.style.color = 'var(--success)';
        feedbackArea.innerHTML = `<div style="color: var(--success); font-size: 24px; font-weight: bold;">✅ Chính xác!</div>`;
      } else {
        ...
        feedbackArea.innerHTML = `
          ...
          <div style="color: var(--text-secondary); font-size: 14px;">${currentWord.ipa || ''}</div>
        `;
      }
